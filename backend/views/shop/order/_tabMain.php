    ...
    <div class="row">
        <div class="col-md-6">
            <div class="btn-group pull-left" role="group" aria-label="toolbar">
                <button type="button" class="btn btn-warning <?=$order->canMakeNew()?'order-change-status':'disabled'?>" data-url="<?=Url::toRoute(['change-status-ajax','id'=>$order->id,'status_id'=>Status::NEW])?>" data-method="POST" title="Статус новый">Статус Черновик</button>
